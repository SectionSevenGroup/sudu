...
export const PAGES = {
  ...
  contact: {
    file: 'src/contact.html', title: 'Contact',
    fields: [
...
export function reindex(site) {
  ...
  for (const slug of order) {
    const p = DATA[slug];
    const dim = site.DIMS && site.DIMS[p.heroSrc];
    // w/h let the Work grid write the thumb's own pixel size onto its <img>.
    names[slug] = { eyebrow: p.eyebrow, title: p.title, location: p.location, thumb: p.heroSrc, ...(dim ? { w: dim[0], h: dim[1] } : {}) };
  }
