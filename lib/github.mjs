...
export function client(env) {
  ...
  async function commit(files, message, expectedParent) {
    ...
    if (expectedParent && parent !== expectedParent) {
      const err = new Error('The draft moved while you were editing.');
      err.code = 'draft-moved';
      throw err;
    }
  ...
  async function state() {
    ...
    if (pr) {
      ...
      const found = (combined.statuses || []).find((s) => s.context.includes('netlify'));
      deploy = {
  ...
  async function publish() {
    const s = await state();
    if (!s.hasChanges) throw new Error('There is nothing to publish.');
    if (s.behindBy > 0) throw new Error('The site changed after this draft was started. Reconcile it before publishing.');
    if (!s.deploy || s.deploy.state !== 'success') throw new Error('The preview is not ready, so there is nothing verified to publish.');
    const head = await refSha(draft);
    ...
    if (!merged.merged) throw new Error(merged.message || 'GitHub declined to merge the draft.');
