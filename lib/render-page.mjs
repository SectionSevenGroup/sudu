...
function renderNode(node, vals, sheet) {
  ...
  for (const { name, value } of node.attrs) {
    ...
    const v = interpolate(value, vals, `${name} on <${tag}>`);
    if (typeof v === 'function') throw new Error(`render-page: ${name} on <${tag}> resolved to a function`);
...
function renderText(txt, vals) {
  ...
  return txt.split(/\{\{([\s\S]+?)\}\}/g).map((p, i) => {
    ...
    if (typeof v === 'object' || typeof v === 'function') throw new Error(`render-page: {{ ${p.trim()} }} is not text; the runtime would insert it as an element`);
    return escText(v);
  }).join('');
}
...
export function evaluateComponent(source, { lang = 'en' } = {}) {
  ...
  const ctx = vm.createContext(Object.assign(Object.create(null), { document: { documentElement: { lang } } }));
